...
g1 <- df %>%
  dplyr::filter(Neuron %in% c(60, 120)) %>%
  pivot_wider(names_from = Neuron, values_from = Spikes) %>%
  ggplot(aes(x = `60`, y = `120`,
             color = GV)) +
  ...
  labs(x = "Neuron 60", y = "Neuron 120") +
  coord_cartesian(xlim = c(0, 60), ylim = c(0, 60)) +
  scale_x_continuous(breaks = c(0, 60, 20)) +
  scale_y_continuous(breaks = c(0, 60, 20)) +
...
df %>% 
  ...
  group_map(~{
    mu <- colMeans(.x[,c("60","90","120")])
    sigma <- cov(.x[,c("60","90","120")])
    if(det(sigma) <= 0){
...
fit <- glm(stim_bin ~ `60` + `90` + `120`, data = df_scatter, family = binomial)

x_seq <- seq(0, 200, length.out = 30)
y_seq <- seq(0, 200, length.out = 30)
z_seq <- seq(0, 200, length.out = 30)
...
ggsave("manifold_90_91.png", manifold, width = 4, height = 1.95, dpi = 300)
...
saveWidget(p2, "scatter_3d_90_91.html", selfcontained = TRUE)
webshot("scatter_3d_90_91.html", "scatter_3d.png", vwidth = 800, vheight = 600)