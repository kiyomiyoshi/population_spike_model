...
for (max_firing_rate in max_firing_seq) {
  ...
  responses_90 <- foreach(
    ...
    resp <- rnbinom(n_neurons, size = 1000000, mu = mu) + rnbinom(n_neurons, size = 1000000, mu = R0) * 5
  ...
  # ---- 120 ----
  responses_120 <- foreach(
    i = 1:n_trials,
    .combine = rbind,
    .packages = "stats"
  ) %dopar% {
    
    input_120 <- rnorm(n_trials, 120, 0)
    idx <- round(input_120[i]) 
    idx <- pmin(pmax(idx, 1), 180)
  ...
  df_tmp <- as.data.frame(rbind(responses_90, responses_120))
...
g3 <- ggplot(df_sum_high, aes(x = Sum_spikes, color = factor(Contrast))) +
  ...
  coord_cartesian(xlim = c(0, 1000), ylim = c(0.0015, 0.05)) +
...
ggsave("90_120.png", blindsight_18, width = 6, height = 4, dpi = 300)
...
final_plot <- wrap_plots(figure_list, nrow = 2, ncol = 2) +
  plot_annotation(title = "90 vs. 120")
ggsave("lfi_90_120.png", final_plot, width = 9, height = 3.5, dpi = 300)