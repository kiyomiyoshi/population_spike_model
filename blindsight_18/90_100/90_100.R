...
contrast <- df$Contrast                                       # ノイズやコヒーレンスに相当する？
...
g3 <- ggplot(df_sum_high, aes(x = Sum_spikes, color = factor(Contrast))) +
  ...
  coord_cartesian(xlim = c(0, 500), ylim = c(0.0015, 0.05)) +
...
ggsave("90_100.png", blindsight_18, width = 6, height = 4, dpi = 300)
...
final_plot <- wrap_plots(figure_list, nrow = 2, ncol = 2) +
  plot_annotation(title = "90 vs. 100")
ggsave("lfi_90_100.png", final_plot, width = 9, height = 3.5, dpi = 300)