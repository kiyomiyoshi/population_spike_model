...
p1 <- p1 %>%
  layout(
    scene = list(
      xaxis = list(title = "Neuron 45",  range = c(0, 10)),
      yaxis = list(title = "Neuron 90",  range = c(0, 10)),
      zaxis = list(title = "Neuron 135", range = c(0, 10)),
      aspectmode = "cube"   
    )
  )
...
x_seq <- seq(0, 15, length.out = 50)
y_seq <- seq(0, 15, length.out = 50)
z_seq <- seq(0, 15, length.out = 50)
...
peb <- wrap_plots(plots, ncol = 4)
ggsave("peb.png", peb, width = 12, height = 3, dpi = 2000)
...
saveWidget(p2, "scatter_3d_peb.html", selfcontained = TRUE)
saveWidget(p3, "scatter_3d_peb_logistic.html", selfcontained = TRUE)