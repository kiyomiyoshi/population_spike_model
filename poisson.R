...
max_firing_rate <- 50                                          # Maximum firing rate of each neuron (30 or 50)
tuning_width <- 25                                             # Tuning width (standard deviation) of each neuron's response curve (30, or 50)
spontaneous_firing_rate <- 0                                   # (0 or 10)
...
orientations <- seq(1, 360, by = 1)                            # Possible orientations (0 to 359 degrees)
preferred_orientations <-  seq(1, 360, length.out = n_neurons) # Preferred orientation of each neuron
max_firing_rate <- 25                                          # Maximum firing rate of each neuron (30 or 50)
tuning_width <- 50                                             # Tuning width (standard deviation) of each neuron's response curve (30, or 50)
spontaneous_firing_rate <- 0                                   # (0 or 10)
...
# Fano factor一定のポアソンモデルを使う限り、総和が同じなら分散も同じになる 
# Fano factorの実験結果自体があるから単にそれ使えばいい

# 負の二項分布（最もよく使われる）
# k → ∞ でポアソンに近づく
# k が小さいほど分散が増える

# Quasi-Poisson モデル（GLMで便利）
# Fano factor > 1 を自然に表現

# fine grained discriminationだと2次元間が相関する(Rahnev open mindと繋がる)
# 刺激のばらつきが2つのチューニングカーブの傘下に入る、入らないで正の相関が出る
# coarse grainedだと相関しない
# Samahaで総和でvisibilityが決まることは報告されている