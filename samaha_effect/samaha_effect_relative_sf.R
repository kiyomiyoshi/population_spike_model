...
#           file = "samaha_effect_df_integrated_rsf.csv", 
#           row.names = FALSE)
...
g3 <- ggplot(df_sum_high, aes(x = Sum_spikes, color = factor(Contrast))) +
  ...
  coord_cartesian(xlim = c(0, 10000), ylim = c(0, 0.1)) +
  ggtitle("High alpha:\nspontaneous firing = tuning peak x 0.03") +
...
g4 <- ggplot(df_sum_low, aes(x = Sum_spikes, color = factor(Contrast))) +
  ...
  coord_cartesian(xlim = c(0, 10000), ylim = c(0, 0.1)) +
  ggtitle("Low alpha:\nspontaneous firing = tuning peak x 0.15") +
  theme(plot.title = element_text(size = 11, face = "bold", hjust = 0.5))
...
g5 <- ggplot(df_summary, aes(x = Contrast, y = DeltaC_pred, color = Alpha)) +
  ...
  theme_minimal() +
  scale_x_log10() +
  scale_y_log10(limits = c(0.03, 45)) +
  ylab("ΔC (JND of d' = 1)") +
  xlab("Contrast") +
  theme(
...
samaha_effect_rsf <- plot_grid(p1, p2, p3, p4, ncol = 1, rel_heights = c(1, 1))
ggsave("samaha_effect_rsf.png", samaha_effect_rsf, width = 7, height = 9, dpi = 300)