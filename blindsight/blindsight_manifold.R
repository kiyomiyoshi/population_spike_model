...
for (g in GV_list) {
  p3 <- p2 %>% add_surface(
    x = planes[[as.character(g)]]$x,
    y = planes[[as.character(g)]]$y,
    z = planes[[as.character(g)]]$z,
    opacity = 0.3,
    showscale = FALSE,
    name = paste("Decision plane", g)
  )
}
...
# save figures
plot_list <- list(g2, g3, g6, g5)
plots <- lapply(plot_list, function(p) {
  p + theme(
      plot.margin = margin(t = 5, r = 5, b = 5, l = 5, unit = "pt")
  )
})
bs <- wrap_plots(plots, ncol = 4)
ggsave("bs.png", bs, width = 9, height = 2.5, dpi = 300)
